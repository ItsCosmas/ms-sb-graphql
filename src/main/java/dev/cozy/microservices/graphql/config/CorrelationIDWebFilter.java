...
public class CorrelationIDWebFilter implements WebFilter {
	...
	public @NonNull Mono<Void> filter(@NonNull ServerWebExchange exchange, @NonNull WebFilterChain chain) {
		...
		if (correlationId == null || correlationId.isBlank()) {
			...
			return Mono.defer(() -> writeGraphQLErrorResponse(exchange)); // Lazy
																			// execution
		}
	...
	private Mono<Void> writeGraphQLErrorResponse(ServerWebExchange exchange) {
		...
		return response
			.writeWith(Mono.just(response.bufferFactory().wrap(ERROR_RESPONSE.getBytes(StandardCharsets.UTF_8))));
	}
